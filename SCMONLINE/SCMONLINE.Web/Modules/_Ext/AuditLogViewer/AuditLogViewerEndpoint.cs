...
namespace _Ext.Endpoints
{
    ...
    public partial class AuditLogViewerController : ServiceEndpoint
    {
        public AuditLogViewerResponse List(IDbConnection connection, AuditLogViewerRequest request)
        {
            ...
            //    && fld.EntityId == request.EntityId).OrderByDescending(x => x.ActionDate).ToList();
            var listRequest = new AuditLogSpListRequest();
            var eqFilter = new Dictionary<string, object>();
            eqFilter.Add("EntityTableName", tableName);
            eqFilter.Add(fld.EntityId.PropertyName, request.EntityId);
            listRequest.EqualityFilter = eqFilter;
            var sort = new SortBy[] { new SortBy(fld.ActionDate.PropertyName, true) };
            listRequest.Sort = sort;
            response.EntityVersions = new AuditLogRepository().List(connection, listRequest).Entities;

            //response.EntityVersions.ForEach(item =>
            //{

            //    var itemJson = JsonConvert.DeserializeObject(item.OldEntity);
            //});
