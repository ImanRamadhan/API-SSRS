...
namespace SCMONLINE.Procurement.Endpoints
{
    ...
    using MyRepository = Repositories.RfqItemRepository;
    using MyRow = Entities.RfqItemRow;
    using SCMONLINE.Procurement.Entities;
    using System.Collections.Generic;

    [RoutePrefix("Services/Procurement/RfqItem"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class RfqItemController : ServiceEndpoint
    {
		...
		public FileContentResult ListExcel(IDbConnection connection, ListRequest request) {
            ...
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.RfqItemColumns));
            var bytes = new ReportRepository().Render(report);
            return ExcelContentResult.Create(bytes, "RfqItemList_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xlsx");
        }

        public FileContentResult ListExcelOwnerEstimate(IDbConnection connection, List<RfqItemRow> data)
        {
            //var data = List(connection, request).Entities;
