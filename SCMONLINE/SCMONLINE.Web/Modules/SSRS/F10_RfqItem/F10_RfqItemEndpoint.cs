...
namespace SCMONLINE.Procurement.Endpoints
{
    ...
    using MyRepository = Repositories.F10_RfqItemRepository;
    using MyRow = Entities.RfqItemRow;
    using System.Collections.Generic;
    using SCMONLINE.Procurement.Entities;

    [RoutePrefix("Services/Procurement/F10_RfqItem"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class F10_RfqItemController : ServiceEndpoint
    {
		...
		public FileContentResult ListExcel(IDbConnection connection, ListRequest request) {
            ...
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.F10_RfqItemColumns));
            var bytes = new ReportRepository().Render(report);
            return ExcelContentResult.Create(bytes, "F10_RfqItemList_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xlsx");
        }
