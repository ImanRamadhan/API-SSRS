...
namespace SCMONLINE.Procurement.Endpoints
{
    ...
    using MyRepository = Repositories.F5_ProcParticipantItemRepository;
    using MyRow = Entities.ProcParticipantItemRow;
    using System.Collections.Generic;
    using SCMONLINE.Procurement.Entities;

    [RoutePrefix("Services/Procurement/F5_ProcParticipantItem"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class F5_ProcParticipantItemController : ServiceEndpoint
    {
        ...
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            ...
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.F5_ProcParticipantItemColumns));
            var bytes = new ReportRepository().Render(report);
            return ExcelContentResult.Create(bytes, "F5_ProcParticipantItemList_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xlsx");
        }
        ...
        public FileContentResult ListExcelBidPrice(IDbConnection connection, List<MyRow> data)
        {
            ...
            includeColumns.Add(fld.RfqItemOrderQuantity.PropertyName);
            includeColumns.Add(fld.RfqItemOrderUnit.PropertyName);

            var report = new DynamicDataReport(data, includeColumns, typeof(Columns.F5_ProcParticipantItemColumns));
            var bytes = new ReportRepository().Render(report);
