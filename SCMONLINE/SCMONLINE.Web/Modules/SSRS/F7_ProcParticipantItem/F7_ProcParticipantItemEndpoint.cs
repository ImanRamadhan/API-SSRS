...
namespace SCMONLINE.Procurement.Endpoints
{
    ...
    using MyRepository = Repositories.F7_ProcParticipantItemRepository;
    using MyRow = Entities.ProcParticipantItemRow;
    using System.Collections.Generic;
    using SCMONLINE.Procurement.Entities;

    [RoutePrefix("Services/Procurement/F7_ProcParticipantItem"), Route("{action}")]
    [ConnectionKey(typeof(MyRow)), ServiceAuthorize(typeof(MyRow))]
    public class F7_ProcParticipantItemController : ServiceEndpoint
    {
        ...
        public FileContentResult ListExcel(IDbConnection connection, ListRequest request)
        {
            ...
            var report = new DynamicDataReport(data, request.IncludeColumns, typeof(Columns.F7_ProcParticipantItemColumns));
            var bytes = new ReportRepository().Render(report);
            return ExcelContentResult.Create(bytes, "F7_ProcParticipantItemList_" + DateTime.Now.ToString("yyyyMMdd_HHmmss") + ".xlsx");
        }
        ...
        public FileContentResult ListExcelBidPrice(IDbConnection connection, List<MyRow> data)
        {
            ...
            var report = new DynamicDataReport(data, includeColumns, typeof(Columns.F7_ProcParticipantItemColumns));
            var bytes = new ReportRepository().Render(report);
